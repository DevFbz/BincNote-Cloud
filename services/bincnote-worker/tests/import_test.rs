use anyhow::Result;
use bincnote_worker::error::WorkerError;
use bincnote_worker::import_worker::report::{ImportNotifier, ImportProgress};
use bincnote_worker::import_worker::worker::{run_import_worker, ImportTask};
use bincnote_worker::s3_client::{BlobMeta, S3Client, S3StreamResponse};
...
//   let consumer_name = "bincnote_worker";
//   let workspace_id = uuid::Uuid::new_v4().to_string();
...
pub fn setup_log() {
  ...
  START.call_once(|| {
    ...
    let mut filters = vec![];
    filters.push(format!("bincnote_worker={}", level));
