...
use bincnote_worker::indexer_worker::{run_background_indexer, BackgroundIndexerConfig};
...
pub async fn create_app(listener: TcpListener, config: Config) -> Result<(), Error> {
  ...
  let indexer_config = BackgroundIndexerConfig {
    enable: bincnote_collaborate::config::get_env_var("APPFLOWY_INDEXER_ENABLED", "true")
      .parse::<bool>()
      .unwrap_or(true),
    open_ai_config,
    azure_ai_config,
