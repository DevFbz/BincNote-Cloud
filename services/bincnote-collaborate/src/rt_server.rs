  ...
  S: CollabStorage,
{
  ...
  tokio::spawn(async move {
    // when bincnote-collaborate start, wait for 60 seconds to start the check. Since no groups will
...
///
/// When bincnote-collaborate is deployed as a standalone service, we can use tokio multi-thread.
